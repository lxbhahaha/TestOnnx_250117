...
public class DemoYoloxHumanDetect
{
    private Yolox yolox;

    public DemoYoloxHumanDetect()
    {
        yolox = new(ConfigScriptablObject.Instance.yoloxModel.bytes, ConfigScriptablObject.Instance.yoloxOptions);
    }
    ...
    public (Rect[], float[]) Run(Texture texture)
    {
        yolox.Run(texture);

        // LogDetectionBox(yolox.Detections);
        var results = yolox.Detections;
        int len = 0;
        ...
        Rect[] resultRect = new Rect[len];
        float[] resultfloat = new float[len];
        int j = 0;
        for (int i = 0; i < results.Length; i++)
        {
            // 只保留person标签的结果
            if (results[i].label != 0) continue;
            resultRect[j] = yolox.ConvertToViewport(results[i].rect);
            resultfloat[j] = results[i].probability;
            j++;
        }

        return (resultRect, resultfloat);
    }
}
