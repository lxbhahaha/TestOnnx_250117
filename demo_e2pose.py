...
def get_args():
    ...
    parser.add_argument(
        ...
        '--image',
        type=str,
        default='Assets/Textures/000000041990.jpg'
    )
    parser.add_argument(
        '-m',
        '--model',
        type=str,
        default='Assets/Models/e2epose_mbnv2_1x3x320x320.with_runtime_opt.ort'
    )
    parser.add_argument(
        '-s',
        '--score_th',
...
def get_output_path(input_path, output_mark='_output'):
    # 分离文件名和扩展名
    dirname, basename = os.path.split(input_path)  # dirname = "/aaa/vvv", basename = "cc.jpg"
    filename, ext = os.path.splitext(basename)  # filename = "cc", ext = ".jpg"

    # 构建新路径
    new_filename = f"{filename}{output_mark}{ext}"  # 新文件名：cc_output.jpg
    new_path = os.path.join(dirname, new_filename)  # 新路径：/aaa/vvv/cc_output.jpg
...
def main():
    ...
    save_path = get_output_path(image_path)
